...
res<-sapply(variables, function(X) ida_trans(b_bact[,X])$const)
...
for(j in 1:length(variables)){
  if(!is.na(res[j])){
    ...
    newlabel <- paste("pseudo-log of",label(b_bact)[variables[j]])
    names(newlabel)<-newname
    x<-pseudo_log(b_bact[[variables[j]]], sigma=res[j], base=10)
    label(x)<-newlabel
    b_bact[[newname]] <- x
...
```{r}
for(j in 1:length(variables)){
  print(ida_plot_univar(b_bact, variables[j], sigma=res[j], n_bars=100))
#  if(!is.na(res[j])){
...
```{r}
save(list(c_bact, bact_variables, sigma_values, bact_transformed), file=here::here("data", "bact_env_c.rda"))
```

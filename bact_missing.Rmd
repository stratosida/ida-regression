...
b_bact %>%
  ...
  mutate(Any_VIP_missing = ifelse(apply(b_bact[,c(demog_vars,pivotal_vars, vip_vars)],1,any_miss),NA,1)) %>%
  ...
  mutate(Any_VIP_leuko_kidney_acute_missing =  ifelse(apply(b_bact[,c(demog_vars,pivotal_vars, vip_vars, leuko_related_vars, leuko_ratio_vars, kidney_related_vars, acute_related_vars)],1,any_miss),NA,1)) %>%
  miss_var_summary() %>%
  gt::gt() %>%
    gt::cols_label(
    variable = md("**Variable**"),

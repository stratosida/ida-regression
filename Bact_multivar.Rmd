...
```{r}
Hmisc::redun(~t_EOS+I(t_BASO)+t_NEU+t_LYM+t_MONO, data=c_bact)
```

Now we add WBC:

```{r}
Hmisc::redun(~t_EOS+I(t_BASO)+t_NEU+t_LYM+t_MONO+t_WBC, data=c_bact)
```
...
```{r}
set.seed(7123981)
mice_obj <- mice::mice(c_bact[,variables], m=1, method="pmm")
```

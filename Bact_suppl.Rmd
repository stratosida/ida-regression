...
set.seed(2)
sel_sample <- as.logical(round(runif(dim(model_df_complete)[1]))) # needed later
...
r_squared_lave <- function(y, prediction){
  n <- length(y)
...
tribble(
  ~data, ~AUC, ~`Lave's R squared`,
  'complete', auc(y_complete, pred_complete) %>% as.numeric(), r_squared_lave(y_complete, pred_complete),
  'central 50%', auc(y_central, pred_central) %>% as.numeric(), r_squared_lave(y_central, pred_central),
  '50% sample', auc(y_sample, pred_sample) %>% as.numeric(), r_squared_lave(y_sample, pred_sample),
  ) %>%
  gt() %>%
...
bind_rows(
  ...
  mutate(model = factor(model, levels = c('complete', 'central 50%', '50% sample'))) %>%

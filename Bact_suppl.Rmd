...
Comparison of models using original vs. pseudo-log transformed covariates. Effect are shown in terms of log odds and on the original and the transformed scale.

```{r trafo of variables, message = FALSE, warning = FALSE, fig.width=12}
...
tidy(fit_mfp_complete) %>% gt 
tidy(fit_mfp_complete_notrans) %>% gt

# medians of all key predicors, selcted variables will be adjusted to these medians in effect plots
...
```{r prediction validity, message = FALSE, warning = FALSE, fig.width=9}
...
To show the effect of multiple imputation if the number of missing values is high, we construct a dataset with 50% missings in one variable. First, recall the output of the complete model, relying on the Bacteremia data with complete cases regarding the key predictors.
...
Creatinine ('KREA') is significant at a level that might not survive if a substantial amount of missing creatinine values. We thus create a dataset were we artificially introduce 50% missing creatinine value, missing completely at random.
...
Next we fit a 'complete case' model in the case of missing creatinine data, using again the fp-transformations from the global model.
...
We now can compare the outputs of the complete model, the complete model with missing data (i.e. only half of the original complete data is used), and the imputed model. (in table: complete / complete, missing / imputated; all numbers are rounded to 3 decimal points)

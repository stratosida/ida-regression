...
n_bacteremia <- sum(model_df_complete$BC)
pct_bateremia <- round((n_bacteremia / dim(model_df_complete)[1])*100,1)

model_df_complete %>%
  select(key_predictors_orig, key_predictors_trans) %>%
  ungroup %>%
  summarise_all(
    ~paste0(median(.) %>% round(2),
            ' (',
            paste0(quantile(., c(.025,.975)) %>% round(2), collapse = ','),
            ')')
  ) %>%
...
In the following examples we use the Bacteremia data with complete observations regarding the key predictors `r paste(key_predictors_orig, collapse = ', ')`, which represent `r round(pct_complete, 1)`% of the whole dataset. We will fit a reference logistic regression model with the outcome 'BC' and the key predictors as covariates. We will use pseudo-log transformations as suggested in the IDA. Within the model, all key predictors will be transformed by fractional polynomials of order 1 (df = 2).

The aim of the examples is to showcase how decisions derived from IDA influence the results of the fitted model.

## S1.1 Reference Model

The reference model will be fitted by the *mfp* function. If not indicated otherwise, we will use the fp-transformations of the key predictors determined in reference model in all consecutive models. For all models we report scaled Brier score and the area under the ROC curve (AUROC), and boxplots comparing predictions of the probability of bacteremia with actual outcomes.

### Model Summary

```{r reference model, message = FALSE, warning = FALSE, fig.width=2, fig.height=3}
...
tidy(fit_mfp_complete) %>%
  gt_model_table(title = 'Reference model')

#AUC
AUC = auc(fit_mfp_complete$y, predict(fit_mfp_complete, type = 'response'))

p_model_complete <- logreg_summary_plot(fit_mfp_complete, 'Reference model')

p_model_complete
```

### Functional forms for reference model

We now take a look at the functional forms of the covariates in the reference model, which are determined by the fp algorithm. Besides scaling factors, only for t_WBC_noNEU the fp algorithm chose a non-linear transformation (note the '^0.5' in the term column). This means all other covariates enter the model in a linear fashion. In the following effect plots, each variable is adjusted to the median of the other variables in the model.

```{r reference model functional forms, message = FALSE, warning = FALSE}
...
## S1.2: to transform or not to transform

Only for one out of the six key predictors did the fp algorithm chose a non-linear transformation. But out of those six variables, four were pseudo-log transformed before entering the model. In the first example we want to compare the reference model to a model using the key predictors on their original scale.

```{r reference model no trans, message = FALSE, warning = FALSE, fig.width=2, fig.height=3}
...
tidy(fit_mfp_complete_notrans) %>% gt_model_table('Model without pseudo-log tranformations')
...
p_model_notrans <- logreg_summary_plot(fit_mfp_complete_notrans, 'Model \nwithout pseudo-log \ntransformations') # maybe side by side with reference model?
...
Next we will compare the differences of the functional forms in the two models for those covariates where a pseudo-log transformation was suggested in IDA. We will look at the log odds for bacteremia by each covariate on the original and the transformed scale, and compare the reference model using the original and the pseudo-log transformed covariates. Each variable is adjusted for the median of all other variables used.
...
## S1.3: the support of a model determines what it can explain

Next we compare the reference model to a model were for an important variable, in our case we chose WBC_noNEU, the variable's support is reduced to the central 50% of the data (i.e. data within the 25% and 75% quantiles). Again, in the reduced models we use the same fp-transformations as in the reference model.

The range and interquartile range of WBC_noNEU are: (`r min(model_df_complete$WBC_noNEU)`, `r max(model_df_complete$WBC_noNEU)`; (`r quantile(model_df_complete$WBC_noNEU, 0.25)`, `r quantile(model_df_complete$WBC_noNEU, 0.75)`).

The range and interquartile range of t_WBC_noNEU are: (`r min(model_df_complete$t_WBC_noNEU)`, `r max(model_df_complete$t_WBC_noNEU)`; (`r quantile(model_df_complete$t_WBC_noNEU, 0.25)`, `r quantile(model_df_complete$t_WBC_noNEU, 0.75)`).
...
## S1.4: the limits of mulitiple imputation
...
tidy(fit_mfp_complete) %>% gt_model_table('reference model')

```

Creatinine's ('CREA') regression coefficient is significant at a level that might not survive substantial missingness. We thus create a dataset were we artificially introduce 50% missing creatinine values, missing completely at random.
...
model_df_missings <- model_df_complete %>%
  mutate(
    t_CREA = ifelse(
      runif(dim(model_df_complete)[1]) < .5,  #~50%/50% TRUE/FALSE
      t_CREA,
      NA
    )
  )
...
Next we fit a 'complete case' model in the case of missing creatinine data, using the fp-transformations from the reference model.

```{r, message = FALSE, warning = FALSE}

fit_mfp_missing <- glm(as.formula(reference_formula_fp), #use same fp-trafos as in reference model
                       data = model_df_missings,
                       family = binomial)

```

Now we impute the missing creatinine data using MICE with 50 imputations, fit the model using the fp-transformations from the reference model and pool the results.
...
imp_fits <- with(imp_data,
                 glm(as.formula(reference_formula_fp), #use same fp-trafos as in reference model
                 family = binomial)
  )
...
bind_rows(
  tidy(fit_mfp_complete)  %>% mutate(model = 'reference model'),
...
The z-statistic for creatinine drops from 2.98 to 1.49 when half the data is missing. Also in other variables the z-statistic is less extreme in the 'missing, complete case analysis' compared to the reference model. The interesting observations is that MI recreates estimates and standard errors very close to the reference model in most variables, but not in the one that was being imputed, namely creatinine. In variable selection, chreatinine, which is highly significant in the 'true' model, is likely to be dropped, based on the imputed data. 

```{r, message = FALSE, warning = FALSE}
#pearson & spearman correlation of CREA and BUN
...
## S1.5: Interpretation of regression coefficient 'size'
...
model_df_complete %>%
  summarise(
    ...
    Age = sd((AGE / 100) * fit_mfp_complete$coefficients[4]),
    CREA = sd((t_CREA) * fit_mfp_complete$coefficients[5]),
...
Because the fp-transformations further complicate the interpretation of the regression coefficients, let us consider two logistic regression models with WBC_noNEU and t_WBC_noNEU as single covariate, respectively.

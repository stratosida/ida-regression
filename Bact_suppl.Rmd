...
bind_rows(
  tibble(
    BC = fit_mfp_complete$y,
    prediction = predict(fit_mfp_complete, type = 'response'),
    model = 'complete'
  ),
  tibble(
    BC = fit_mfp_complete$y[fit_mfp_complete$X[,'Alter'] %>% between(
    quantile(fit_mfp_complete$X[,'Alter'], .25), quantile(fit_mfp_complete$X[,'Alter'], .75))],
    prediction = predict(fit_mfp_complete, new_data_central_50 %>% rename(Alter = x_trans), type = 'response'),
    model = 'central 50%'
  ),
  tibble(
    BC = fit_mfp_complete$y[sel_50_pct],
    prediction = predict(fit_mfp_complete, new_data_sample_50  %>% rename(Alter = x_trans), type = 'response'),
    model = 'sample 50%'
  ),
  ) %>%
  mutate(model = factor(model, levels = c('complete', 'central 50%', 'sample 50%'))) %>%
  ggplot(aes(x = BC, y = prediction, group = BC)) +
  geom_boxplot() + 
  facet_grid(~model) +
  theme_minimal()

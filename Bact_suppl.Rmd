...
In the following examples we use the Bacteremia data with complete observations regarding the key predictors `r paste(key_predictors_orig, collapse = ', ')`, which represent `r round(pct_complete, 1)`% of the whole dataset. We will fit a global logistic regression model with the outcome 'BC' and the key predictors as covariates. We will use pseudo-log transformations as suggested in the IDA. All key predictors will be transformed by fractional polynomials of order 1 (df = 2). If not indicated otherwise, we will use the fp-transformations of the key predictors determined in global model in all consecutive models.

The aim of the examples is to showcase how decisions derived for the IDA influence the results of the fitted model.

## Global Model

The global model will be fitted by the *mfp* function. For all models we report McFaddens's R² and the AUC, i.e. the area under the ROC curve, and boxplots comparing BC predictions with outcomes.
...
Only for one out of the six key predictors did the fp algorithm chose a non-linear transformation. But out of those six variables, four were pseudo-log transformed before entering the model. In the first example we want to compare global model to a model using the key predictors on their original scale.
...
#tidy(fit_mfp_complete) %>% gt 
tidy(fit_mfp_complete_notrans) %>% gt_model_table('global model without pseudo-log tranformations')

```
Note the different fp transformation arising when the key predictors are not pseudo-log transformed. On the original scale, three covariates instead of one now enter the model via a non-linear fp transformation. This suggests that a transfromation prior to the regression model 'outsources' the need for fp transformations. Now let us compare the model performances.
...
Next we will compare the differences of the functional forms in the two models for those covariates where a pseudo-log transformation was suggested. We will look at the log odds for bacteremia by each covariate on the original and the transformed scale, and compare the global model using the original and the pseudo-log transformed covariates. Each variable is adjusted for the median of all other variables used.
...
## Example2: the support of a model determines what it can explain

Comparison of support vs. extrapolations. For each key predictor we create a dataset which includes the median +/- 25% of the complete data and compare the fitted effects of the full vs. the reduced model on the range of the complete data. Note that in the reduced models we use the same fp-transformations as in the complete model.
...
tidy(fit_mfp_part) %>%
  gt_model_table('part of data used')
...
p <- plot_df %>%
  mutate(
    model = case_when(
      ...
      model == 'part' ~ paste0('data within \n', 50 - m_pct*100/2, '% quantile and\n', 50 + m_pct*100/2, '% quantile')
    )
  ) %>%
  ggplot(aes(x = x_trans, y = yhat, ymin = yhat.lwr, ymax = yhat.upr, color = model, fill = model)) +
  annotate(
...
p_model_restricted <- logreg_summary_plot(fit_mfp_part, 'restricted model')

print(p + (p_model_complete + p_model_restricted))
...
The estimates $-0.56$ and $-2.92$ denote the change in log odds for the outcome when the 'term' variable changes by 1 unit, but cannot be compared directly. A $1$ unit change is only a small step on the original scale, where WBC_noNEU covers values from `r range(model_df_complete$WBC_noNEU)[1]` up to `r range(model_df_complete$WBC_noNEU)[2]`. In comparison, t_WBC_noNEU lies between `r range(model_df_complete$t_WBC_noNEU)[1]` up to `r range(model_df_complete$t_WBC_noNEU)[2]`, so change of $1$ unit cover almost half the range of the variable. 
